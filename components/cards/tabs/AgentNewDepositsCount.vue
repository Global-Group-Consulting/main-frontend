...
export default defineComponent({
  ...
  setup (props, ctx) {
    ...
    const dashboardTabLogic = useDashboardTabLogic<AgentNewDepositsCountDto[]>(ctx, props, sections)

    dashboardTabLogic.setFetchFunction(async (filters) => {
      return $apiCalls.statisticsApi.getAgentNewDepositsCount(filters)
    })
    ...
    dashboardTabLogic.setOnDataChange((newData) => {
      ...
      newData.forEach((section) => {
        ...
        sections.value.push({
          id: section._id,
          title: moneyFormatter(section.totalAmount),
